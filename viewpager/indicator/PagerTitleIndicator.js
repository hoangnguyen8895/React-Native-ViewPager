...
export default class PagerTitleIndicator extends Component {
  ...
  render() {
    ...
    let titleViews = titles.map((title, index) => {
      ...
      const itemMargin =
        (itemStyle && itemStyle.marginLeft) || DEFAULT_ITEM_MARGIN;
      ...
      const titleView = this.props.renderTitle ? (
        ...
        <Text
          style={
            isSelected
              ? [styles.titleTextSelected, selectedItemTextStyle]
              : [styles.titleText, itemTextStyle]
          }
        >
          {title}
        </Text>
      ...
      return (
        ...
        >
          {titleView}
          {isSelected ? (
            <View style={[styles.selectedBorder, selectedBorderStyle]} />
          ) : null}
        </TouchableOpacity>
      );
    });
  ...
  _visibleDetect(selectedIndex) {
    ...
    if (moveDir === FORWARD) {
      ...
      if (
        this._contentHorOffset > lastItemOffsetX ||
        this._currentMaxHor < curItemAbsPosition
      ) {
