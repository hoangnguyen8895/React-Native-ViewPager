...
export default class XNavigator extends React.Component {
  ...
  render() {
    let sencePaddingTop = this.props.showNavigationBar
      ? XNavBarStyle.General.TotalNavHeight
      : 0;
    return (
      <Navigator
        {...this.props}
        ...
        sceneStyle={StyleSheet.flatten([
          XNavigatorStyles.scene,
          { paddingTop: sencePaddingTop },
  ...
  componentWillMount() {
    if (Platform.OS === "android") {
      BackAndroid.addEventListener(
        "hardwareBackPress",
        this._routeRenderer.onBackPress
      );
    }
  }
